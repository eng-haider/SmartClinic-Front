<template>
  <div>
    
    <!-- Simple Bottom Sheet for Categories -->
    <v-bottom-sheet v-model="showContextMenu" max-width="600">
      <v-card class="category-bottom-sheet">
        <!-- Header -->
        <v-card-title class="d-flex justify-space-between align-center pa-3 bg-primary">
          <div class="d-flex align-center text-white">
            <v-icon color="white" class="mr-2">mdi-tooth</v-icon>
            <span class="text-subtitle-1">{{ $t('teeth.tooth_number') }} {{ formatToothLabel(contextTooth) }}</span>
          </div>
          <v-btn 
            icon 
            size="small" 
            variant="text"
            color="white"
            @click="hideContextMenu"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
        </v-card-title>

        <!-- Search -->
        <v-card-text class="pa-3">
          <v-text-field
            v-model="categorySearch"
            :placeholder="$t('teeth.search_category')"
            prepend-inner-icon="mdi-magnify"
            variant="outlined"
            density="compact"
            hide-details
            clearable
            class="mb-3"
          ></v-text-field>

          <!-- Categories Grid -->
          <div class="categories-simple-grid">
            <v-btn
              v-for="cat in displayedCategories"
              :key="cat.id"
              @click="selectCategory(cat)"
              variant="tonal"
              color="primary"
              class="category-simple-btn"
              block
            >
              <v-icon start size="18">mdi-medical-bag</v-icon>
              <span class="text-truncate">{{ cat.name || cat.name_ar }}</span>
            </v-btn>
          </div>

          <!-- No Results -->
          <div v-if="displayedCategories.length === 0" class="text-center py-8">
            <v-icon size="48" color="grey-lighten-1">mdi-medical-bag-outline</v-icon>
            <p class="text-body-2 text-grey mt-2">{{ categorySearch ? $t('teeth.no_matching_categories') : $t('teeth.no_categories') }}</p>
          </div>

          <!-- Loading -->
          <div v-if="loadingCategories" class="text-center py-8">
            <v-progress-circular indeterminate color="primary"></v-progress-circular>
          </div>
        </v-card-text>
      </v-card>
    </v-bottom-sheet>

    <div>
      <div class="teeth-svg" :style="{ '--chart-scale': chartScale }">

        <svg version="1.1" class="toomain" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" :viewBox="viewBox"
          style="direction: rtl !important;" xml:space="preserve">

          <a href="#">
            <g>
              <path class="st0" d="M6.3,9.7C7.6,9.4,9,9.3,10.4,9.3c139.7,0,279.4,0,419.1,0c14.2,0.1,28.3-0.2,42.5,0.2c0,5.5-0.1,11,0,16.5
    c-6.6-3-14-6-21.4-4c-4.1,1.3-6.5,5.3-8.4,8.9c-2-3.6-4.4-7.7-8.7-8.8c-4.8-1.1-10.1-0.9-14.7,1c-2.9,1.2-4.4,4.2-6,6.6
    c-2.1-4.7-5.2-9.6-10.4-11.1c-4-0.8-8.6-0.5-12,2c-2.7,2.1-4.1,5.3-5.8,8.3c-4-5.8-10.4-11.6-18.1-10.1c-5.7,0.9-8.3,6.6-10.8,11.1
    c-2.4-5.9-6.1-12-12.3-14.5c-7.7-1.6-12.6,6.1-16.6,11.3c-0.9-3.2-1.6-6.7-3.9-9.2c-2.4-2.7-5.6-4.8-9-5.9c-3.1-1-6.3,0.3-9,1.7
    c-4.6,2.4-6.4,7.6-8.1,12.1c-4.1-5.2-10.4-9.7-17.3-8.5c-5.3,0.7-9.4,4.6-11.9,9.1c-2.8-6.6-8.3-13.4-16-13.7
    c-6.3,0.1-9.9,5.8-12.7,10.7c-2.7-4.8-6.1-10.6-12.4-10.6c-7.9,0.1-13.3,7.1-16.2,13.7c-2.5-4.2-6-8-11-8.9
    c-7.2-1.9-13.8,2.9-18.2,8.1c-2-5.1-4.3-11-10-12.9c-5.3-2.9-11.2,0.4-15.1,4.1c-3,2.6-4,6.6-5.2,10.2c-2.8-4.1-6-8.2-10.4-10.7
    c-2.9-1.3-6.6-1-9.1,1.1c-4.4,3.1-7.2,8-9.3,12.8c-2.4-4.5-5-10.1-10.6-11.1c-7.7-1.5-14.1,4.4-18.2,10.3c-1.9-3.8-3.9-8.1-8.2-9.7
    c-4.4-1.6-10-1.5-13.8,1.6c-3,2.2-4.7,5.5-6.2,8.8c-1.7-2.7-3.5-5.9-6.8-6.9c-5.1-1.7-11-2-16,0.2c-3.4,1.4-4.9,4.9-6.5,7.9
    c-2.1-3.5-4.5-7.5-8.6-8.9c-7.3-1.9-14.7,0.8-21.2,4.1C6,20.6,5.8,15.2,6.3,9.7z" />
            </g>
          </a>
          <g>
            <a href="#">
              <g>
                <path class="st1" d="M156.2,16.5c3.9-3.7,9.8-7,15.1-4.1c5.7,1.9,8.1,7.8,10,12.9c4.4-5.2,11-10,18.2-8.1c4.9,1,8.5,4.8,11,8.9
        c2.9-6.6,8.3-13.7,16.2-13.7c6.2,0.1,9.7,5.9,12.4,10.6c2.8-4.8,6.4-10.6,12.7-10.7c7.7,0.3,13.2,7.1,16,13.7
        c2.5-4.5,6.6-8.3,11.9-9.1c6.9-1.3,13.2,3.3,17.3,8.5c1.7-4.5,3.5-9.7,8.1-12.1c2.7-1.4,6-2.7,9-1.7c3.4,1.2,6.6,3.2,9,5.9
        c2.3,2.6,3,6,3.9,9.2c4-5.2,8.9-12.9,16.6-11.3c6.2,2.5,9.9,8.6,12.3,14.5c2.5-4.5,5.1-10.2,10.8-11.1c7.6-1.5,14,4.3,18.1,10.1
        c1.6-2.9,3-6.2,5.8-8.3c3.4-2.5,8-2.8,12-2c5.2,1.5,8.3,6.4,10.4,11.1c1.6-2.5,3-5.5,6-6.6c4.6-1.9,9.9-2.1,14.7-1
        c4.3,1.1,6.7,5.1,8.7,8.8c2-3.6,4.3-7.6,8.4-8.9c7.4-2.1,14.8,0.9,21.4,4c1,3.6,0.7,7.3,0.8,11c-0.1,2.4,0.3,5.1-1.4,7
        c-3.3,1.8-7.2,1.4-10.8,1.1c-3.5-0.5-5.9,2.5-8.9,3.5c-3.1,0.5-6-0.9-8.8-2c-9,3.5-18.9,3.2-28.3,1.7c-4.6,3.5-11.2,3.3-15.4-0.7
        c-2.4,1.3-4.6,3.3-7.5,3.2c-3.4,0.3-5.6-2.7-7.8-4.8c-5.6,2.3-11.8,6.3-17.9,3.8c-3.4-1.2-6.5-2.9-9.5-4.8
        c-4.6,3.8-10.3,7.4-16.6,6.6c-5.5-0.7-9.8-4.6-13.2-8.7c-2.9,4.8-7.2,9.5-12.8,10.8c-6.2,0.6-10.4-5-14.5-8.7
        c-1.8,2.4-3.8,4.9-6.8,5.8c-7.2,2.6-15.3,2.2-22.3-0.6c-1.9,2.8-5,4.2-8.3,4.4c-7.5,0.8-15.6,1.6-22.6-1.8
        c-6.7,3.3-14.4,2.6-21.6,2c-3.6-0.3-7.4-1.3-9.4-4.5c-9,3.5-20.5,3.7-28.1-3.1c-0.1-0.7-0.2-2.2-0.2-2.9
        c-3.9,3.7-7.8,8.4-13.3,9.6c-6.6,0-11.3-5.8-14.9-10.7c-3.8,4-8.1,8.1-13.9,8.5c-6.1,0.6-11.2-3.2-15.9-6.5
        c-4.3,2.1-8.4,4.9-13.2,5.5c-5.1-0.2-9.7-2.6-14.3-4.7c-2.1,2.1-4.4,5.1-7.7,4.8C83.6,51,81.3,49,79,47.7
        c-4.3,4-10.9,4.1-15.5,0.7c-9.4,1.5-19.3,1.8-28.3-1.8c-3.7,1.8-8.4,3.5-12,0.5c-4.8-4-11.4,0.2-16.6-3c-3.2-4.7-1-10.8-1.5-16.1
        c0.3-0.5,0.8-1.4,1.1-1.8c6.4-3.3,13.9-6,21.2-4.1c4.1,1.5,6.5,5.4,8.6,8.9c1.6-3,3.1-6.5,6.5-7.9c5-2.2,10.9-1.9,16-0.2
        c3.3,1,5,4.2,6.8,6.9c1.5-3.3,3.2-6.6,6.2-8.8c3.8-3.1,9.3-3.3,13.8-1.6c4.2,1.6,6.3,5.9,8.2,9.7c4.1-5.8,10.4-11.8,18.2-10.3
        c5.6,1,8.2,6.6,10.6,11.1c2.2-4.9,4.9-9.7,9.3-12.8c2.5-2.1,6.2-2.4,9.1-1.1c4.4,2.6,7.6,6.6,10.4,10.7
        C152.2,23.1,153.1,19.1,156.2,16.5z M162.9,14.5c-9.8,4.3-10.7,16.3-9.7,25.6c1.9,5.1,6,9,10.8,11.4c2.8,1.1,5.2-1.3,7.4-2.7
        c3.5-2.8,7.2-6.2,8.3-10.7c0.7-7.5-0.8-15.5-5.8-21.4C170.9,14,166.5,12.6,162.9,14.5z M224.1,14.8c-8.7,3.5-13.2,13.3-12.9,22.2
        c0.2,4.7-0.4,9.5,0.4,14.1c3.4,1.8,7.3,1.3,10.9,1.9c4.1,0.7,8.3,0,12.3-0.7c3.6-1,3-5.3,3.1-8.2c-0.3-8.2,1.3-17.4-3.9-24.5
        C231.9,16.2,228,13.5,224.1,14.8z M250,14.7c-3.1,0.8-5.1,3.5-6.8,6.1c-4.4,7-2.7,15.4-3.1,23.2c0,2.8-0.5,6.4,2.4,8
        c4.9,1.3,10.1,1.5,15.1,0.7c3-0.4,7.1,0.3,9.1-2.5c0.3-4.4,0-8.8,0.1-13.2c0.1-7.1-2.4-14.6-8-19.3
        C256.5,15.7,253.3,13.8,250,14.7z M304.6,16.2c-5.4,5.8-7,14.3-6.2,21.9c1,4.3,4.4,7.5,7.7,10.2c2.3,1.6,4.8,4.3,7.8,3.1
        c4.9-2.3,8.7-6.5,11-11.3c0.2-8.2,0.6-18.2-6.8-23.6C314.5,13,308.5,12.8,304.6,16.2z M133.8,18.4c-8.5,5.2-11,16.1-9.4,25.4
        c2.5,3,6.4,4.5,10.1,5.3c5.6,1,11-2.3,14.1-6.8c2.9-3,1.9-7.5,1.6-11.3c-2.2-4.7-5.2-9.3-9.6-12.2C138.6,17.4,136,16.9,133.8,18.4
        z M184.3,25.8c-3.8,5.9-1.3,13.3-2.2,19.8c2.3,2.3,5.3,3.7,8.4,4.2c6,0.8,12.5,0.7,17.8-2.6c0.3-5.8,0.4-11.6-0.1-17.3
        c-0.4-4.5-3.5-8.2-7.4-10.1C194.7,17.5,187.8,20.6,184.3,25.8z M338.7,18.1c-5,2.5-8.2,7.4-10.7,12.3c-0.9,3.1-0.3,6.4-0.4,9.6
        c2,2.7,3.8,5.6,6.9,7.2c6,4,14.5,1.8,19.1-3.4c1.5-7.9-0.1-16.9-6.1-22.8C345.2,18.9,341.9,16.2,338.7,18.1z M270.7,26.4
        c-2.1,6.7-1.4,14-0.8,21c5.8,3.3,12.9,3.4,19.2,2.1c3-1,7.6-2.4,6.8-6.5c-0.2-6.9,1.5-15.6-5-20.2C285.1,17,273.6,18.3,270.7,26.4
        z M99.3,25.8c-4.8,4.4-4.9,11.5-4.3,17.5c2.1,1.9,4.8,2.9,7.4,4.1c2.8,0.4,5.7,1.4,8.5,0.4c3.7-1.2,7.9-2.6,10.1-6
        c0.4-3.6,0.7-7.6-1.2-10.8c-1.9-3.8-4-8.3-8.2-9.9C106.9,19.5,102.5,22.7,99.3,25.8z M366.5,20.9c-4.3,1.6-6.4,6.1-8.4,10
        c-2,3.4-1.2,7.6-1.2,11.3c2.5,2.2,5.4,4.1,8.7,5.1c6,2.3,12.6-0.6,17.6-4c0.1-5.6,0.6-12-3.4-16.5
        C376.3,23.4,371.7,19.4,366.5,20.9z M66.3,34.6c-0.9,4.2-0.3,8.6-0.4,12.9c3.9,1.4,8.3,1.3,11.5-1.5c-1.1-1-2.1-2-3.1-3
        c2.9-1.8,5.1,1.5,7,3.2c2.3,2.4,6.4,3.4,8.8,0.6c2.7-2.1,2-5.7,2.1-8.7c0.1-5-1.7-10-4.6-14.1c-2.5-3.4-7.2-4.1-11.2-3.2
        C70.7,22.9,68,29.1,66.3,34.6z M390.3,24c-4.3,5.7-5.3,13.1-4.5,20c1.1,2.9,4.5,5.6,7.6,4.1c3.8-1.1,5.4-6.4,9.9-5.7
        c-0.4,1.6-1.7,2.6-2.7,3.8c3.3,2.6,7.5,2.7,11.3,1.3c-0.2-5.2,0.8-10.6-1.2-15.5c-1.6-4.4-4.1-8.9-8.5-10.9
        C398.2,19.7,393,20.4,390.3,24z M7.7,28c0.2,4.5-0.3,9.1,0.3,13.6c1.6,1.6,3.9,1.7,6,1.5c0.1-0.7,0.3-2.2,0.4-2.9
        c3.6,1.7,7.5,2.9,10.7,5.3c2.1,1.8,5.3,1.2,7.4-0.2c2.3-1.7,1.2-4.9,1.5-7.3c0.5-5.7-2.3-12.5-8.3-14.2C19.5,23.1,13,24.7,7.7,28z
         M40.8,27.4c-3.2,5.1-5,11.3-3.9,17.4c5.8,2.7,12.5,2.7,18.8,2.6l0.7-0.5c2.2-0.1,4.4-0.2,6.6-0.4c-0.1-6.8,1.6-15-3.5-20.5
        C53.8,23.3,45.3,22.1,40.8,27.4z M420.2,24.7c-7.3,4.6-5.1,14.4-5.2,21.6c8.4,1.1,17.3,1.9,25.4-1.2c1.6-0.6,0.7-2.8,1-4.1
        c0.1-4.8-1.7-9.6-4.2-13.6C433.1,22.3,425.8,23.8,420.2,24.7z M448.5,25.9c-5.1,4.3-4.8,11.6-4.4,17.7c1.1,2.9,4.7,3,7.4,3.1
        c3.5-3.3,8.1-4.8,12.5-6.4c-0.1,0.8-0.2,2.3-0.2,3c2-0.4,4.9,0.2,6.3-1.8c0.7-4.5,0.1-9,0.2-13.6c-3.6-2.2-7.6-3.5-11.7-4.1
        C455.1,23.6,451,23.1,448.5,25.9z" />
              </g>
            </a>
            <path class="st1" d="M75.6,111.7c2.1-0.8,5.3-5.5,6.9-1.8c-3,1.5-5.4,3.9-7,6.9c-1.7-2.2-3.4-4.2-5-6.4
    C72.3,110.2,73.9,111.2,75.6,111.7z" />
          </g>                    <a href="#">
              <g>
                <path class="comon st2" @click="reply_click(13, $event)" @contextmenu.prevent="right_click(13, $event)" @touchstart="reply_click(13, $event)"
                  :class="[isToothActive(13) ?  'activeClass comon st3' : 'comon st3']" id="13"
                  d="M162.9,14.5c3.6-1.9,8.1-0.5,10.9,2.1c5,5.9,6.5,13.9,5.8,21.4c-1.1,4.5-4.7,7.9-8.3,10.7
        c-2.2,1.4-4.5,3.8-7.4,2.7c-4.8-2.3-8.9-6.3-10.8-11.4C152.2,30.9,153.1,18.8,162.9,14.5z" />
              </g>
            </a>
            <a href="#">
              <g>
                <path @click="reply_click(11, $event)" @contextmenu.prevent="right_click(11, $event)" @touchstart="reply_click(11, $event)"
                  :class="[isToothActive(11) ?  'activeClass comon st3' : 'comon st3']" id="11"
                  d="M224.1,14.8c4-1.2,7.9,1.4,9.8,4.8c5.2,7.1,3.6,16.3,3.9,24.5c-0.1,2.9,0.5,7.2-3.1,8.2
        c-4,0.7-8.2,1.4-12.3,0.7c-3.7-0.5-7.5-0.1-10.9-1.9c-0.8-4.6-0.1-9.4-0.4-14.1C210.9,28,215.4,18.2,224.1,14.8z" />
                <path @click="reply_click(21, $event)" @contextmenu.prevent="right_click(21, $event)" @touchstart="reply_click(21, $event)"
                  :class="[isToothActive(21) ?  'activeClass comon st3' : 'comon st4']" id="21" d="M250,14.7c3.3-0.9,6.5,1,8.8,3c5.5,4.7,8,12.1,8,19.3c-0.1,4.4,0.1,8.8-0.1,13.2c-2,2.8-6.1,2.1-9.1,2.5
        c-5,0.8-10.2,0.5-15.1-0.7c-2.8-1.6-2.3-5.3-2.4-8c0.3-7.7-1.3-16.2,3.1-23.2C244.9,18.2,246.9,15.4,250,14.7z" />
                <path @click="reply_click(43, $event)" @contextmenu.prevent="right_click(43, $event)"
                  :class="[isToothActive(43) ?  'activeClass comon st5' : 'comon st5']" id="43"
                  d="M163.9,102.6c2.9-0.8,5.4,1.5,7.6,3.2c3.1,2.7,6.1,5.6,8.2,9.2c-0.1,6.1-0.3,12.8-4.5,17.7c-2.2,3-6.3,4-9.7,3
        c-5.8-2-10.5-6.8-12.3-12.6c0-4.3-1.2-9.1,1.4-12.9C156.7,106.7,160,103.8,163.9,102.6z" />
                <path @click="reply_click(42, $event)" @contextmenu.prevent="right_click(42, $event)"
                  :class="[isToothActive(42) ?  'activeClass comon st5' : 'comon st6']" id="42"
                  d="M192.5,104.1c4.7-0.4,9.5-0.3,14.1,0.8c1.3,0,1.9,0.7,1.8,1.9c0.2,6.8,0.3,13.8-2.2,20.3
        c-1,3.4-4.6,5.1-7.7,6.2c-5.1,0.9-10.3-1.9-13.4-5.9c-4.1-5.8-2.8-13.2-2.9-19.8C184.4,104.4,189,104.8,192.5,104.1z" />
                <path @click="reply_click(41, $event)" @contextmenu.prevent="right_click(41, $event)"
                  :class="[isToothActive(41) ?  'activeClass comon st7' : 'comon st7']" id="41"
                  d="M211.2,105.7c8.5-2.8,17.9-3.1,26.6-0.7c0,5.7,0.2,11.4-0.1,17c-1.1,5.3-4.8,10.6-10.1,12.4
        c-3.1,0.4-5.9-1.6-8.1-3.5c-3.4-3.3-5.8-7.4-7.3-11.9C210.5,114.8,211.4,110.1,211.2,105.7z" />
              </g>
            </a>
            <a href="#">
              <g>
                <path class="comon st8" @click="reply_click(23, $event)" @contextmenu.prevent="right_click(23, $event)"
                  :class="[isToothActive(23) ?  'activeClass comon st3' : 'comon st3']" id="23" d="M304.6,16.2c3.9-3.4,9.8-3.2,13.6,0.3c7.3,5.4,7,15.5,6.8,23.6c-2.3,4.9-6,9.1-11,11.3
        c-3.1,1.2-5.6-1.5-7.8-3.1c-3.3-2.7-6.7-6-7.7-10.2C297.6,30.4,299.2,22,304.6,16.2z" />
                <path class="comon st9" id="14" @click="reply_click(14, $event)" @contextmenu.prevent="right_click(14, $event)"
                  :class="[isToothActive(14) ?  'activeClass comon st3' : 'comon st3']" d="M133.8,18.4c2.1-1.6,4.8-1,6.8,0.4c4.4,2.9,7.4,7.5,9.6,12.2c0.3,3.7,1.3,8.3-1.6,11.3
        c-3.1,4.4-8.6,7.8-14.1,6.8c-3.7-0.8-7.6-2.3-10.1-5.3C122.8,34.5,125.3,23.6,133.8,18.4z" />


                <path class="comon st10" id="12" @click="reply_click(12, $event)" @contextmenu.prevent="right_click(12, $event)"
                  :class="[isToothActive(12) ?  'activeClass comon st3' : 'comon st3']" d="M184.3,25.8c3.6-5.1,10.4-8.2,16.6-6c3.9,1.9,7,5.6,7.4,10.1c0.4,5.8,0.3,11.6,0.1,17.3
        c-5.3,3.3-11.8,3.4-17.8,2.6c-3.2-0.5-6.2-1.9-8.4-4.2C182.9,39,180.5,31.7,184.3,25.8z" />


                <path class="comon st11" id="24" @click="reply_click(24, $event)" @contextmenu.prevent="right_click(24, $event)"
                  :class="[isToothActive(24) ?  'activeClass comon st3' : 'comon st3']" d="M338.5,18.1c3.2-1.9,6.5,0.8,8.8,3c5.9,5.9,7.6,14.9,6.1,22.8c-4.6,5.1-13.1,7.4-19.1,3.4
        c-3-1.6-4.9-4.5-6.9-7.2c0.1-3.2-0.5-6.5,0.4-9.6C330.3,25.5,333.5,20.6,338.5,18.1z" />
                <path class="comon st12" id="22" @click="reply_click(22, $event)" @contextmenu.prevent="right_click(22, $event)"
                  :class="[isToothActive(22) ?  'activeClass comon st3' : 'comon st3']" d="M270.7,26.4c2.9-8.1,14.4-9.4,20.2-3.7c6.6,4.7,4.8,13.3,5,20.2c0.8,4.1-3.8,5.5-6.8,6.5
        c-6.4,1.3-13.4,1.2-19.2-2.1C269.3,40.4,268.6,33.1,270.7,26.4z" />
                <path class="comon st13" id="44" @click="reply_click(44, $event)" @contextmenu.prevent="right_click(44, $event)"
                  :class="[isToothActive(44) ?  'activeClass comon st3' : 'comon st3']" d="M131.1,106c6.1-3.2,13.9-0.8,17.4,5c2.5,2.4,1.8,5.9,1.9,9c0.4,4-2.5,7.2-4.9,10c-3,3.5-8.5,5-12.6,2.3
        c-4.1-3-8.4-6.9-8.8-12.3c0-3.1-0.8-6.6,0.9-9.5C126.7,108.5,128.9,107,131.1,106z" />
                <path class="comon st14" id="34" @click="reply_click(34, $event)" @contextmenu.prevent="right_click(34, $event)"
                  :class="[isToothActive(34) ?  'activeClass comon st3' : 'comon st3']"
                  d="M335.9,105.4c4.5-1.8,9.8-0.6,13.7,2.1c2.1,1.5,4.3,3.6,4.3,6.4c0.3,4.8-0.1,10.2-3.7,13.7
        c-2.7,2.6-5.6,6.2-9.7,5.9c-6.2,0.3-10.4-5.6-12.7-10.6c-0.1-4-1.3-8.8,1.7-12C331.1,108.6,333.2,106.6,335.9,105.4z" />
              </g>
            </a>
            <a href="#">
              <g>
                <path class="comon st15" @click="reply_click(15, $event)" @contextmenu.prevent="right_click(15, $event)"
                  :class="[isToothActive(15) ?  'activeClass comon st3' : 'comon st3']" id="15"
                  d="M99.3,25.8c3.3-3.1,7.6-6.3,12.3-4.8c4.2,1.6,6.4,6.1,8.2,9.9c1.9,3.3,1.6,7.2,1.2,10.8
        c-2.3,3.4-6.4,4.9-10.1,6c-2.8,1-5.7,0-8.5-0.4c-2.6-1.1-5.3-2.2-7.4-4.1C94.4,37.3,94.5,30.1,99.3,25.8z" />
                <path class="comon st16" id="25" @click="reply_click(25, $event)" @contextmenu.prevent="right_click(25, $event)"
                  :class="[isToothActive(25) ?  'activeClass comon st3' : 'comon st3']" d="M366.5,20.9c5.2-1.5,9.9,2.5,13.3,5.9c4,4.5,3.5,10.9,3.4,16.5c-5,3.4-11.5,6.3-17.6,4
        c-3.3-1-6.1-2.8-8.7-5.1c0-3.8-0.8-7.9,1.2-11.3C360,27,362.1,22.5,366.5,20.9z" />
                <path v-if="showTooth(16)" class="comon st17" id="16" @click="reply_click(16, $event)" @contextmenu.prevent="right_click(16, $event)"
                  :class="[isToothActive(16) ?  'activeClass comon st3' : 'comon st3']" d="M66.3,34.6c1.7-5.5,4.4-11.7,10.1-13.8c3.9-0.9,8.6-0.2,11.2,3.2c2.9,4.1,4.7,9,4.6,14.1
        c-0.1,3,0.7,6.6-2.1,8.7c-2.4,2.8-6.6,1.8-8.8-0.6c-2-1.7-4.2-5-7-3.2c1,1,2.1,2,3.1,3c-3.3,2.8-7.7,2.9-11.5,1.5
        C66,43.2,65.4,38.8,66.3,34.6z" />
                <path v-if="showTooth(28)" class="comon st18" id="28" @click="reply_click(28, $event)" @contextmenu.prevent="right_click(28, $event)"
                  :class="[isToothActive(28) ?  'activeClass comon st3' : 'comon st3']" d="M448.5,25.9c2.6-2.8,6.6-2.3,10-2.1c4.1,0.6,8.1,2,11.7,4.1c-0.1,4.5,0.4,9.1-0.2,13.6
        c-1.4,1.9-4.2,1.3-6.3,1.8c0.1-0.8,0.2-2.3,0.2-3c-4.4,1.6-9,3.2-12.5,6.4c-2.7-0.1-6.3-0.2-7.4-3.1
        C443.6,37.5,443.4,30.2,448.5,25.9z" />
                <path class="comon st19" id="33" @click="reply_click(33, $event)" @contextmenu.prevent="right_click(33, $event)"
                  :class="[isToothActive(33) ?  'activeClass comon st3' : 'comon st3']" d="M298.4,114.8c3-4.8,7.3-9.2,12.3-11.9c2.5-1.4,5.2,0.1,7.3,1.5c3.2,2,5.3,5.2,7,8.4c-0.3,4.4,1,9.3-1.4,13.3
        c-2.3,4-5.9,7.3-10.1,9.1c-3.3,1.4-7.6,0.8-10-2.1C298.8,128.3,298.1,121.2,298.4,114.8z" />
                <path class="comon st20" id="45" @click="reply_click(45, $event)" @contextmenu.prevent="right_click(45, $event)"
                  :class="[isToothActive(45) ?  'activeClass comon st3' : 'comon st3']" d="M106.7,104.3c5.4-0.9,10.7,2.8,13.6,7.2c1.5,1.8,1,4.4,1,6.5c-0.1,5.3-3.9,9.7-8,12.7
        c-2.7,2.6-7.3,2.7-10.3,0.6c-3.4-2.1-6.4-5-8-8.7c-0.4-3.4-0.3-6.9-0.1-10.3C97.1,107.8,102,105.3,106.7,104.3z" />
                <path class="comon st21" id="35" @click="reply_click(35, $event)" @contextmenu.prevent="right_click(35, $event)"
                  :class="[isToothActive(35) ?  'activeClass comon st3' : 'comon st3']" d="M366,104.8c6.2-2,13.4,1.7,16.8,7c0.7,3.5,0.4,7.2,0.2,10.8c-2,4.5-5.9,8.2-10.4,10
        c-5.7,1.6-10.2-3.2-13.3-7.3c-2.8-3.6-2.9-8.5-2.1-12.8C359.1,109,362.2,106.1,366,104.8z" />
              </g>
            </a>
            <a href="#">
              <g>
                <path v-if="showTooth(26)" class="comon st22" id="26" @click="reply_click(26, $event)" @contextmenu.prevent="right_click(26, $event)"
                  :class="[isToothActive(26) ?  'activeClass comon st3' : 'comon st3']" d="M390.3,24c2.7-3.6,7.9-4.2,11.9-2.9c4.4,2,7,6.5,8.5,10.9c2,4.9,1,10.3,1.2,15.5c-3.8,1.4-8,1.3-11.3-1.3
        c1-1.2,2.4-2.2,2.7-3.8c-4.4-0.7-6,4.6-9.9,5.7c-3.2,1.5-6.6-1.2-7.6-4.1C385,37,386,29.7,390.3,24z" />
              </g>
            </a>
            <a href="#">
              <g>
                <path v-if="showTooth(18)" class="comon st23" id="18" @click="reply_click(18, $event)" @contextmenu.prevent="right_click(18, $event)"
                  :class="[isToothActive(18) ?  'activeClass comon st3' : 'comon st3']" d="M7.7,28c5.3-3.3,11.7-4.9,18-4.1c6,1.6,8.7,8.4,8.3,14.2c-0.3,2.4,0.9,5.6-1.5,7.3c-2.1,1.4-5.4,2-7.4,0.2
        c-3.2-2.5-7.1-3.7-10.7-5.3c-0.1,0.7-0.3,2.2-0.4,2.9c-2.1,0.2-4.4,0.1-6-1.5C7.4,37.1,7.9,32.5,7.7,28z" />
              </g>
            </a>
            <a href="#">
              <g>
                <path v-if="showTooth(17)" class="comon st24" id="17" @click="reply_click(17, $event)" @contextmenu.prevent="right_click(17, $event)"
                  :class="[isToothActive(17) ?  'activeClass comon st3' : 'comon st3']" d="M40.8,27.4c4.6-5.3,13-4.1,18.8-1.6c5.1,5.5,3.4,13.7,3.5,20.5c-2.2,0.2-4.4,0.4-6.6,0.4l-0.7,0.5
        c-6.3,0.1-13,0.1-18.8-2.6C35.8,38.7,37.6,32.5,40.8,27.4z" />
              </g>
            </a>
            <a href="#">
              <g>
                <path v-if="showTooth(27)" class="comon st25" id="27" @click="reply_click(27, $event)" @contextmenu.prevent="right_click(27, $event)"
                  :class="[isToothActive(27) ?  'activeClass comon st3' : 'comon st3']" d="M420.2,24.7c5.6-0.9,12.9-2.4,17,2.7c2.5,4.1,4.3,8.8,4.2,13.6c-0.3,1.3,0.6,3.5-1,4.1
        c-8.1,3.1-17,2.3-25.4,1.2C415,39.1,412.9,29.3,420.2,24.7z" />

              </g>
            </a>

            <a href="#">
              <g>

                <path class="comon st26" id="31" @click="reply_click(31, $event)" @contextmenu.prevent="right_click(31, $event)"
                  :class="[isToothActive(31) ?  'activeClass comon st3' : 'comon st3']" d="M240.2,105c8.6-2.5,18-2.1,26.5,0.6c0,3.5,0,6.9,0,10.4c-2,7-5.6,14.3-12.4,17.6c-2.9,1.8-6.4,0.2-8.6-1.9
        c-2.6-2.7-4.8-6-5.5-9.7C240.1,116.3,240.3,110.7,240.2,105z" />
              </g>
            </a>
            <g>
              <a href="#">
                <g>
                  <path class="st27" d="M151.4,110.3c3.2-5.2,8.4-10.7,15.1-9.9c5.5,1.2,9.1,6,13.1,9.7c0.1-1.4,0.3-2.9,0.4-4.3
          c2.6-1.6,5.3-3.1,8.3-3.5c7.2-1,14.8-1,21.8,1.3c9.1-3.3,19.2-2.9,28.6-0.8c9.7-1.8,20.2-2.7,29.6,1c1-0.3,2.1-0.6,3.2-0.9
          c6.4-0.7,12.9-0.5,19.3-0.1c3.7,0.9,8.1,2.7,8.1,7.1c3.7-3.7,7.4-8.2,12.7-9.5c6.7-0.8,11.8,4.8,15.1,9.8c3.5-4.1,8.1-8.2,13.9-8
          c6-0.2,11.3,3.5,15.2,7.7c3.5-4.2,8.1-8.2,13.9-8.2c5.8,0.3,11,3.6,15,7.6c2-1.8,4-4,6.9-4.1c3.9-0.7,7.7,1,11.4,2
          c2.1,0,4.2-1,6.3-0.8c1.9,0.9,3.3,2.6,4.8,4.1c4.3-3.4,10.1-2.8,14.6-0.2c5-2.8,10.7-1.6,15.2,1.5c4.6-0.8,9.4-0.9,13.9,0.3
          c4.1-1.5,9.5-3.2,13.1,0.3c3.4,5,2,11.7,0.2,17c-0.3,0.3-1,0.8-1.3,1.1c-5.8,5-14.8,6.4-21.6,2.8c-3.2-1.7-4.8-5.2-6.3-8.3
          c-3.5,5-8.7,9.9-15.2,9.6c-5.6,0.6-9.2-4.4-12.3-8.3c-1.4,2.8-2.5,6.3-5.8,7.3c-5.7,1.9-12.1,1.7-17.9,0.2
          c-3.4-0.8-5.2-4.1-6.9-6.8c-3.6,3.5-7.6,7.7-13.1,7.7c-6.8,0.5-11.3-5.6-14.9-10.5c-3.2,5.2-7.7,10.4-14,11.6
          c-6.7,0.8-11.8-4.5-15.2-9.6c-3,6-8.6,10.2-14.9,12.3c-1.4,0.1-2.8,0.1-4.1,0c-5.5-1.9-8.8-6.9-10.6-12.2c-3.1,5.1-8.2,9-14.2,9.7
          c-3.8,0.2-7.5-1.3-10.3-3.7c-3-2.5-3.6-6.7-4.7-10.2c-3.1,6.3-7.8,13-15.1,14.6c-6.2,0.5-10.2-5.2-13.5-9.6
          c-3,4.6-7.2,10-13.3,9.5c-7.2-1.6-12.1-8.1-15-14.4c-1.2,5.2-3.7,10.8-9.3,12.5c-7.8,3.4-16-2-20.2-8.4
          c-1.7,5.6-5.5,10.9-11.6,12.3c-1.1,0.1-2.1,0.1-3.2,0c-6.3-2-11.6-6.5-14.8-12.3c-3.3,4.7-7.8,9.9-14.2,9.6
          c-6.8-0.5-11.7-6.3-15.2-11.7c-3.3,5.1-8,10.9-14.7,10.6c-5.5,0-9.5-4.3-13.3-7.8c-1.6,2.4-3,5.4-5.9,6.4
          c-5.7,1.9-12,2.1-17.9,0.5c-3.5-0.8-5.1-4.5-6.9-7.2c-3,3.3-6,7.7-11,7.8c-7.1,0.7-13.3-4.3-16.5-10.2c-1,5-4.8,9.3-9.9,10.5
          c-6.9,1.4-15.1-0.1-19.4-6.3c-2.4-5-2.9-11.4,0.3-16.1c3.7-3.4,9-1.6,13.1-0.2c4.5-1.3,9.3-1.2,13.9-0.3
          c4.4-3.3,10.3-4.3,15.3-1.6c4.5-2.7,10.3-3.2,14.6,0.2c1.4-1.5,2.9-2.9,4.3-4.4c2.1,0.3,4.2,0.9,6.3,1.1c3.9-1,7.8-2.9,12-2.1
          c2.8,0.2,4.7,2.4,6.7,4.1c4-4.1,9.2-7.4,15-7.7c5.7,0.1,10.4,4.1,13.9,8.3c3.8-4.4,9.1-7.9,15.1-7.8
          C143.3,102,148,106.1,151.4,110.3z M163.9,102.6c-3.9,1.2-7.2,4.1-9.3,7.5c-2.6,3.8-1.4,8.6-1.4,12.9c1.8,5.8,6.5,10.6,12.3,12.6
          c3.4,1.1,7.5,0,9.7-3c4.2-4.9,4.4-11.6,4.5-17.7c-2-3.6-5.1-6.5-8.2-9.2C169.3,104.1,166.8,101.8,163.9,102.6z M298.4,114.8
          c-0.2,6.4,0.4,13.5,5,18.4c2.4,2.8,6.7,3.5,10,2.1c4.3-1.8,7.9-5.1,10.1-9.1c2.5-4,1.1-8.8,1.4-13.3c-1.7-3.3-3.8-6.4-7-8.4
          c-2.1-1.4-4.9-2.9-7.3-1.5C305.6,105.7,301.4,110,298.4,114.8z M106.7,104.3c-4.7,1-9.6,3.5-11.7,8c-0.3,3.4-0.4,6.9,0.1,10.3
          c1.6,3.7,4.6,6.7,8,8.7c3,2.1,7.6,2,10.3-0.6c4-3.1,7.9-7.4,8-12.7c-0.1-2.2,0.5-4.7-1-6.5C117.4,107.1,112.1,103.3,106.7,104.3z
           M192.5,104.1c-3.4,0.7-8,0.3-10.2,3.6c0.1,6.6-1.2,14,2.9,19.8c3.1,4,8.3,6.9,13.4,5.9c3.1-1.1,6.7-2.8,7.7-6.2
          c2.5-6.5,2.4-13.5,2.2-20.3c0.1-1.3-0.5-1.9-1.8-1.9C202,103.9,197.2,103.7,192.5,104.1z M211.2,105.7c0.2,4.5-0.7,9.1,1,13.4
          c1.5,4.4,3.9,8.6,7.3,11.9c2.3,1.9,5,3.9,8.1,3.5c5.3-1.8,9-7.1,10.1-12.4c0.3-5.7,0.1-11.4,0.1-17
          C229.1,102.5,219.7,102.8,211.2,105.7z M240.2,105c0,5.7-0.2,11.4,0,17c0.7,3.7,2.9,7,5.5,9.7c2.2,2.1,5.7,3.8,8.6,1.9
          c6.8-3.3,10.4-10.7,12.4-17.6c0-3.5,0-6.9,0-10.4C258.3,102.8,248.9,102.5,240.2,105z M366,104.8c-3.8,1.3-6.9,4.2-8.9,7.7
          c-0.8,4.3-0.7,9.2,2.1,12.8c3.1,4.1,7.6,8.9,13.3,7.3c4.6-1.8,8.4-5.5,10.4-10c0.2-3.6,0.5-7.3-0.2-10.8
          C379.4,106.4,372.2,102.8,366,104.8z M131.1,106c-2.3,1.1-4.5,2.5-6,4.6c-1.7,2.9-0.9,6.4-0.9,9.5c0.4,5.4,4.7,9.3,8.8,12.3
          c4,2.7,9.5,1.2,12.6-2.3c2.4-2.8,5.3-6,4.9-10c-0.1-3.1,0.6-6.6-1.9-9C145,105.2,137.3,102.8,131.1,106z M269.2,105.7
          c0,7.7-0.3,15.7,2.9,22.9c3,5.3,10.7,6.7,15.8,3.5c4.8-2.6,7.9-7.7,8.1-13.1c0.1-3.8,0-7.7,0.1-11.5c-2.3-1.4-4.7-2.6-7.4-2.8
          C282.1,104.1,275.5,104.1,269.2,105.7z M335.9,105.4c-2.7,1.1-4.8,3.2-6.5,5.5c-3,3.2-1.8,8-1.7,12c2.3,5.1,6.5,10.9,12.7,10.6
          c4.2,0.3,7-3.3,9.7-5.9c3.6-3.5,4.1-8.9,3.7-13.7c0.1-2.8-2.1-5-4.3-6.4C345.7,104.8,340.4,103.6,335.9,105.4z M74.7,110.9
          c-1.9-1.2-4.7-3.7-6.6-1.1c-3.7,3-1.8,8.1-2.3,12.2c-0.4,5,3,11.1,8.7,10.7c5.1-0.2,11.8,1.4,15.4-3.2c3.3-4.1,2.1-9.7,2.3-14.5
          c0.5-3.2-2-5.8-4.5-7.3C83.1,106.3,78.3,107.9,74.7,110.9z M385.7,115c0.2,4.8-1,10.3,2.3,14.4c2.3,3.4,6.8,3.3,10.4,3.4
          c3.8-0.1,8.5,0.7,11.2-2.7c4.1-5.1,1.8-11.9,2.4-17.8c-1.3-1.4-2.7-2.7-4.1-4c-2,0.6-4,3.6-6.1,1.7c-2.4-1.7-5.3-2.9-8.3-2.7
          C389.3,106.7,385.3,110.9,385.7,115z M36.7,113.1c-0.6,5.4,1.3,10.8,4.9,14.8c3.4,3.1,8.1,6,12.8,4.3c5.4-2.9,9.3-9,8.8-15.2
          c-0.2-1.6,0.4-3.6-1.1-4.7c-3.5-3.5-9.3-1.8-12.6,1.3c-2.1-1.5-4.4-2.9-7.1-2.9C40.4,111.2,38.5,112.2,36.7,113.1z M415,113.3
          c-0.6,4.4-0.4,9.1,2.2,12.9c2.4,3.3,5.8,7.1,10.3,6.4c6.4-1,11.5-6.3,13.2-12.5c0.8-2.9,1.8-7.7-2.2-8.5
          c-3.4-1.9-7.2-0.3-10.1,1.9C424.9,110.2,418.2,108.8,415,113.3z M457.6,115c-3.3-1.8-7.1-2.3-10.7-1.6c-1.1,0.5-2.8,1.1-2.9,2.6
          c-0.4,5-0.3,11,4.1,14.4c3.8,3.9,9.8,2.8,14.6,1.8c7.7-2.7,10.5-14.3,4.5-19.9C463.8,112.3,460.5,113.3,457.6,115z M7.7,120
          c-0.6,5.8,3.7,12.1,9.7,12.7c4.2,0.8,9.3,1.2,12.5-2.3c4.7-3.6,4.5-10.2,3.8-15.4c-3.7-3.1-9.3-1.9-13.3,0.1
          c-2.8-1.5-5.8-2.9-9.1-2.6C8.5,113.8,7.3,117,7.7,120z" />
              </g>
            </a>
            <path class="st27" d="M395.5,109.9c0.9-3.4,3.8-0.2,5.3,0.8c1.9,2.5,4.3-0.9,6.7-0.3c-1.6,2.2-3.4,4.3-5,6.4
    C400.9,113.8,398.5,111.4,395.5,109.9z" />
            <path class="st27" d="M17.3,118.1c3.5-2.9,8.1-3.5,12.5-3c-0.2,0.5-0.7,1.7-0.9,2.2c-1.8,0-3.6-0.1-5.3,0.1
    C21.5,117.8,19.4,119.7,17.3,118.1z" />
          </g>
          <g>
            <a href="#">
              <g>
                <path v-if="showTooth(38)" class="comon st28" @click="reply_click(38, $event)" @contextmenu.prevent="right_click(38, $event)"
                  :class="[isToothActive(38) ?  'activeClass comon st3' : 'comon st3']"
                  id="38" d="M457.6,115c2.9-1.8,6.2-2.8,9.7-2.7c6,5.7,3.2,17.2-4.5,19.9c-4.8,1-10.8,2.2-14.6-1.8
            c-4.4-3.3-4.5-9.4-4.1-14.4c0.1-1.5,1.7-2.1,2.9-2.6C450.5,112.8,454.3,113.3,457.6,115z M448,115.2c0.3,0.7,0.6,1.4,0.9,2.1
            c3.5-0.2,7.1,0,10.2,2c0.4-0.8,0.8-1.5,1.1-2.3C456.3,115.2,452.1,114.4,448,115.2z" />
              </g>
            </a>
          </g>
          <a href="#">
            <g>
              <path class="st29" d="M448,115.2c4.2-0.8,8.4,0,12.2,1.9c-0.4,0.8-0.8,1.5-1.1,2.3c-3-2-6.7-2.2-10.2-2
    C448.5,116.6,448.2,115.9,448,115.2z" />
            </g>
          </a>
          <a href="#">
            <g>
              <path class="comon st30"
                d="M271.4,102.9c6.3-1.6,12.9-1.5,19.3-0.1C284.3,102.3,277.8,102.2,271.4,102.9z" />
            </g>
          </a>
          <a href="#">
            <g>
              <path class="comon st31" @click="reply_click(32, $event)" @contextmenu.prevent="right_click(32, $event)"
                :class="[isToothActive(32) ?  'activeClass comon st3' : 'comon st3']" id="32"
                d="M269.2,105.7c6.3-1.6,12.9-1.5,19.3-1c2.7,0.2,5.2,1.4,7.4,2.8c-0.1,3.8,0,7.7-0.1,11.5
        c-0.2,5.4-3.3,10.5-8.1,13.1c-5,3.1-12.8,1.8-15.8-3.5C268.9,121.4,269.2,113.3,269.2,105.7z" />
            </g>
          </a>
          <g>
            <a href="#">
              <g>
                <path v-if="showTooth(46)" class="comon st32" @click="reply_click(46, $event)" @contextmenu.prevent="right_click(46, $event)"
                  :class="[isToothActive(46) ?  'activeClass comon st3' : 'comon st3']"
                  id="46" d="M74.7,110.9c3.6-3,8.4-4.6,13-3.3c2.5,1.5,4.9,4.1,4.5,7.3c-0.3,4.8,1,10.5-2.3,14.5
            c-3.6,4.6-10.3,3-15.4,3.2c-5.6,0.4-9.1-5.7-8.7-10.7c0.4-4.1-1.4-9.2,2.3-12.2C70,107.2,72.8,109.8,74.7,110.9z M75.6,111.7
            c-1.7-0.5-3.3-1.5-5.1-1.3c1.6,2.2,3.4,4.3,5,6.4c1.6-2.9,4-5.4,7-6.9C80.9,106.2,77.7,110.8,75.6,111.7z" />
              </g>
            </a>
          </g>
          <g>

            <a href="#">
              <g>
                <path v-if="showTooth(36)" class="comon st33" @click="reply_click(36, $event)" @contextmenu.prevent="right_click(36, $event)"
                  :class="[isToothActive(36) ?  'activeClass comon st3' : 'comon st3']"
                  id="36"
                  d="M385.7,115c-0.4-4.1,3.6-8.3,7.8-7.8c3-0.3,5.8,1,8.3,2.7c2.1,1.9,4.1-1.1,6.1-1.7c1.5,1.2,2.8,2.6,4.1,4
            c-0.6,5.9,1.7,12.7-2.4,17.8c-2.7,3.4-7.4,2.6-11.2,2.7c-3.6,0-8.1,0-10.4-3.4C384.7,125.4,385.9,119.8,385.7,115z M395.5,109.9
            c3,1.5,5.4,3.9,7,6.8c1.7-2.2,3.4-4.2,5-6.4c-2.4-0.6-4.8,2.8-6.7,0.3C399.3,109.7,396.4,106.5,395.5,109.9z" />
              </g>
            </a>

          </g>
          <a href="#">
            <g>

              <path v-if="showTooth(47)" class="comon st34" @click="reply_click(47, $event)" @contextmenu.prevent="right_click(47, $event)"
                :class="[isToothActive(47) ?  'activeClass comon st3' : 'comon st3']" id="47"
                d="M36.7,113.1c1.9-0.9,3.8-1.9,5.8-2.4c2.6-0.1,5,1.4,7.1,2.9c3.3-3,9.1-4.7,12.6-1.3c1.5,1.1,0.9,3.1,1.1,4.7
        c0.4,6.2-3.4,12.3-8.8,15.2c-4.7,1.7-9.4-1.2-12.8-4.3C38,123.9,36,118.5,36.7,113.1z" />
              <path v-if="showTooth(37)" class="comon st35" @click="reply_click(37, $event)" @contextmenu.prevent="right_click(37, $event)"
                :class="[isToothActive(37) ?  'activeClass comon st3' : 'comon st3']" id="37"
                d="M415,113.3c3.2-4.6,10-3.2,13.5,0.3c2.8-2.2,6.6-3.8,10.1-1.9c4,0.8,3,5.6,2.2,8.5
        c-1.7,6.2-6.8,11.5-13.2,12.5c-4.5,0.7-8-3.1-10.3-6.4C414.6,122.4,414.4,117.7,415,113.3z" />
            </g>
          </a>
          <g>
            <a href="#">
              <g>
                <path v-if="showTooth(48)" class="comon st36" @click="reply_click(48, $event)" @contextmenu.prevent="right_click(48, $event)"
                  :class="[isToothActive(48) ?  'activeClass comon st3' : 'comon st3']"
                  id="48" d="M7.7,120c-0.4-3,0.9-6.2,3.7-7.6c3.3-0.3,6.3,1.2,9.1,2.6c4-1.9,9.6-3.2,13.3-0.1c0.6,5.3,0.9,11.8-3.8,15.4
            c-3.2,3.4-8.3,3-12.5,2.3C11.4,132.1,7.1,125.8,7.7,120z M17.3,118.1c2.1,1.6,4.2-0.2,6.3-0.6c1.8-0.2,3.6-0.1,5.3-0.1
            c0.2-0.6,0.7-1.7,0.9-2.2C25.4,114.5,20.8,115.1,17.3,118.1z" />
              </g>
            </a>
          </g>
          <a href="#">
            <g>
              <path class="st0" d="M210.6,122.2c2.9,6.4,7.8,12.9,15,14.4c6.1,0.5,10.3-5,13.3-9.5c3.3,4.4,7.3,10,13.5,9.6
        c7.4-1.5,12-8.2,15.1-14.6c1.1,3.6,1.7,7.7,4.7,10.2c2.9,2.4,6.6,3.9,10.3,3.7c6-0.7,11.1-4.6,14.2-9.7c1.8,5.3,5.2,10.3,10.6,12.2
        c-1.9,0.5-3.9,1-5.9,0.9c-39.7-0.1-79.4-0.1-119-0.1c-4.3,0-8.7,0.5-12.9-0.8c6.1-1.4,9.8-6.7,11.6-12.3
        c4.2,6.4,12.4,11.8,20.2,8.4C206.9,133,209.3,127.4,210.6,122.2z" />
            </g>
          </a>
          <a href="#">
            <g>
              <path class="st0" d="M36.1,124.4c3.2,5.9,9.4,10.9,16.5,10.2c4.9-0.1,8-4.5,11-7.8c1.8,2.8,3.4,6.4,6.9,7.2
        c5.8,1.5,12.2,1.4,17.9-0.5c2.9-1,4.3-4,5.9-6.4c3.8,3.5,7.8,7.8,13.3,7.8c6.7,0.3,11.4-5.5,14.7-10.6c3.5,5.4,8.3,11.2,15.2,11.7
        c6.3,0.3,10.8-4.9,14.2-9.6c3.2,5.8,8.5,10.2,14.8,12.3c-1.6,0.5-3.2,0.8-4.9,0.9c-51.8-0.1-103.7,0-155.5-0.1
        c-0.1-3.6-0.2-7.4,0.9-10.9c4.2,6.2,12.4,7.7,19.4,6.3C31.3,133.7,35,129.4,36.1,124.4z" />
              <path class="st0" d="M341.6,135.9c6.3-1.1,10.8-6.4,14-11.6c3.6,4.9,8.1,11,14.9,10.5c5.5,0,9.5-4.2,13.1-7.7
        c1.7,2.7,3.5,6,6.9,6.8c5.8,1.6,12.1,1.7,17.9-0.2c3.3-1.1,4.4-4.5,5.8-7.3c3.1,3.9,6.7,8.8,12.3,8.3c6.6,0.3,11.7-4.6,15.2-9.6
        c1.5,3.2,3,6.6,6.3,8.3c6.8,3.6,15.8,2.2,21.6-2.8l1.6-1.1c0.1,3.3,0.1,6.6,0,9.9c-48.6,0-97.2,0-145.8,0c-4.7-0.1-9.5,0.6-14-0.9
        c6.3-2.1,11.9-6.3,14.9-12.3C329.8,131.3,334.9,136.7,341.6,135.9z" />
            </g>
          </a>
          <rect :x="numberStrip.x" y="79.3" :width="numberStrip.width" height="0.9" />
          <rect
            v-for="divider in dividerLines"
            :key="`divider-${divider.x}`"
            :x="divider.x"
            :y="divider.y"
            width="0.9"
            height="34.7"
          />
          <text
            v-for="label in toothLabels"
            :key="`label-${label.num}`"
            :transform="`matrix(1 0 0 1 ${label.x} ${label.y})`"
            :class="['st39', 'st40', textClass]"
          >{{ label.display }}</text>
        </svg>

      </div>
    </div>

    <!-- No-Tooth Categories Section -->
    <div v-if="categoriesWithoutTooth.length > 0" class="no-tooth-mobile mt-3 mx-2">
      <div class="no-tooth-mobile-header">
        <v-icon size="16" color="teal">mdi-clipboard-list-outline</v-icon>
        <span>إجراءات لا تتطلب تحديد سن</span>
      </div>
      <div class="no-tooth-mobile-chips">
        <v-btn
          v-for="cat in categoriesWithoutTooth"
          :key="cat.id"
          @click="selectCategoryWithoutTooth(cat)"
          variant="tonal"
          color="teal"
          size="small"
          class="no-tooth-mobile-btn"
        >
          <v-icon start size="16">mdi-plus-circle-outline</v-icon>
          {{ cat.name || cat.name_ar }}
        </v-btn>
      </div>
    </div>
  </div>
</template>

<style>
    .st0 {
        fill: #F37574;
    }

    .st1 {
        fill: #1E1B19;
    }

    .comon {
        fill: #fff;
        stroke: #ddd;
        stroke-width: 1px;
        cursor: pointer;
        transition: all 0.2s ease;
    }

    .st39 {
        font-family: 'TypoRoundBoldDemo';
    }

    .st40 {
        font-size: 12px;
    }

    .comon:hover {
    fill: #0ff !important;
        stroke: #0ff !important;
        stroke-width: 2px !important;
        filter: drop-shadow(0 2px 6px rgba(76, 175, 80, 0.4));
      
    }
    
    /* Add a subtle animation on right-click to provide visual feedback */
    .comon:active {
        fill: #bbdefb;
        stroke: #1976d2;
        stroke-width: 2px;
        transform: scale(0.98);
        transition: all 0.1s ease;
    }
    
    /* Mobile-friendly touch styles */
    @media (max-width: 768px) or (pointer: coarse) {
        .comon {
            cursor: pointer;
            -webkit-tap-highlight-color: transparent;
            stroke-width: 1.5px;
        }
        
        .comon:active {
            fill: #e1f5fe;
            stroke: #0277bd;
            stroke-width: 2.5px;
            transform: scale(1.02);
            transition: all 0.1s ease;
            filter: drop-shadow(0 3px 8px rgba(2, 119, 189, 0.3));
        }
    };

    .activeClass {
        fill: #0ff !important;
        stroke: #0ff !important;
        stroke-width: 2px !important;
        filter: drop-shadow(0 2px 6px rgba(76, 175, 80, 0.4));
    }


.text-light-mode {
fill: #000;
}

.text-dark-mode {
    fill: #fff; /* White for dark mode */
}

.category-bottom-sheet {
  border-radius: 16px 16px 0 0 !important;
  max-height: 80vh;
  overflow: hidden;
}

.category-bottom-sheet :deep(.v-card-title) {
  color: white !important;
}

.category-bottom-sheet :deep(.v-card-text) {
  color: #1e293b !important;
}

.category-bottom-sheet :deep(.v-text-field) {
  color: #1e293b !important;
}

.category-bottom-sheet :deep(.v-text-field .v-field__input) {
  color: #1e293b !important;
}

.category-bottom-sheet :deep(.v-btn) {
  color: #1e293b !important;
}

/* No-Tooth Mobile Section */
.no-tooth-mobile {
  border-top: 1px dashed rgba(0, 150, 136, 0.5);
  padding-top: 10px;
}

.no-tooth-mobile-header {
  display: flex;
  align-items: center;
  gap: 6px;
  color: #00796b;
  font-size: 0.78rem;
  font-weight: 600;
  margin-bottom: 8px;
  padding: 0 4px;
}

.no-tooth-mobile-chips {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
  padding: 0 4px 8px;
}

.no-tooth-mobile-btn {
  text-transform: none;
}

.categories-simple-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(140px, 1fr));
  gap: 8px;
  max-height: 50vh;
  overflow-y: auto;
  padding: 4px;
}

.category-simple-btn {
  text-transform: none !important;
  height: 44px !important;
  font-size: 13px !important;
  justify-content: flex-start !important;
  padding: 0 12px !important;
  color: #1e293b !important;
}

.category-simple-btn .v-btn__content {
  width: 100%;
  justify-content: flex-start;
}

@media (max-width: 600px) {
  .categories-simple-grid {
    grid-template-columns: 1fr;
    gap: 6px;
  }
  
  .category-simple-btn {
    height: 48px !important;
    font-size: 14px !important;
  }
}

/* Scrollbar for category grid */
.categories-simple-grid::-webkit-scrollbar {
  width: 6px;
}

.categories-simple-grid::-webkit-scrollbar-track {
  background: #f1f5f9;
  border-radius: 3px;
}

.categories-simple-grid::-webkit-scrollbar-thumb {
  background: #cbd5e1;
  border-radius: 3px;
}

.categories-simple-grid::-webkit-scrollbar-thumb:hover {
  background: #94a3b8;
}

.tooth-info .v-icon {
  margin-left: 8px;
}

.close-btn {
  color: white !important;
}

.search-section {
  padding: 16px;
  border-bottom: 1px solid #f1f5f9;
  background: #ffffff;
}

.category-search {
  font-family: 'Cairo', sans-serif;
  background: #f5f5f5 !important;
}

.category-search :deep(.v-field__input) {
  color: #1a1a1a !important;
  font-size: 13px !important;
}

.category-search :deep(.v-field__input::placeholder) {
  color: #999 !important;
}

.category-search :deep(.v-field) {
  background: #f5f5f5 !important;
}

.quick-categories, .all-categories {
  padding: 16px;
  background: #ffffff;
}

.section-title {
  display: flex;
  align-items: center;
  font-weight: 700;
  font-size: 13px;
  color: #1a1a1a !important;
  margin-bottom: 12px;
  text-transform: uppercase;
  letter-spacing: 0.5px;
}

.categories-grid {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}

.category-chip {
  margin: 0 !important;
  height: 32px !important;
  font-size: 12px !important;
  border-radius: 16px !important;
  transition: all 0.2s ease;
  cursor: pointer;
  color: #1565c0 !important;
  background: #e3f2fd !important;
}

.category-chip:hover {
  transform: translateY(-2px);
  background: #bbdefb !important;
  box-shadow: 0 4px 12px rgba(21, 101, 192, 0.2);
}

.categories-list {
  max-height: 200px;
  overflow-y: auto;
}

.category-list-item {
  border-radius: 8px;
  margin-bottom: 4px;
  transition: all 0.2s ease;
}

.category-list-item:hover {
  background-color: #f1f5f9;
  transform: translateX(-2px);
}

.action-buttons {
  padding: 12px 16px;
  border-top: 1px solid #f1f5f9;
  background: #fafbfc;
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.add-category-section {
  padding: 16px;
  border-top: 1px solid #f1f5f9;
  background: linear-gradient(135deg, #f8fffe 0%, #f0fdf4 100%);
  border-bottom-left-radius: 12px;
  border-bottom-right-radius: 12px;
}

.add-category-section .section-title {
  color: #059669;
  margin-bottom: 16px;
}



/* Make teeth elements more touch-friendly */
.comon {
  touch-action: manipulation;
  -webkit-tap-highlight-color: transparent;
  vector-effect: non-scaling-stroke;
}

.comon:hover {
  opacity: 0.9;
}

.comon:active {
  opacity: 0.8;
}

.teeth-svg {
  width: var(--chart-scale, 100%);
  max-width: 100%;
  margin: 0 auto;
}

/* Mobile responsiveness for teeth SVG */
@media (max-width: 768px) {
  .toomain {
    max-width: 100%;
    height: auto;
  }
  
  .comon {
    stroke-width: 2px;
  }
  
  .comon:hover,
  .comon:active {
    stroke-width: 3px;
  }
}
</style>

<script>
    import { PRIMARY_TOOTH_OFFSET, PRIMARY_LAST_POSITION, formatToothNumber } from './toothNotation'
    import { useClinicSettings } from '@/composables/useClinicSettings'

    // Number strip under the arch - taken from the artwork so every label keeps
    // sitting under the tooth it belongs to.
    const TOOTH_LABELS = [
        { num: 18, x: 22.3, y: 73.9892 },
        { num: 17, x: 55.4868, y: 73.9892 },
        { num: 16, x: 84.0802, y: 73.9892 },
        { num: 15, x: 114.6549, y: 75 },
        { num: 14, x: 144.7977, y: 75 },
        { num: 13, x: 169.5968, y: 74.9999 },
        { num: 12, x: 198.8225, y: 74.9999 },
        { num: 11, x: 229.3609, y: 74.9999 },
        { num: 21, x: 256.9392, y: 75 },
        { num: 22, x: 286.082, y: 75 },
        { num: 23, x: 316.4399, y: 73.0106 },
        { num: 24, x: 343.2068, y: 73.0106 },
        { num: 25, x: 373.6457, y: 73.0106 },
        { num: 26, x: 403.5746, y: 73.0106 },
        { num: 27, x: 433.4513, y: 73.0106 },
        { num: 28, x: 464.0921, y: 73.0106 },
        { num: 48, x: 22.3, y: 93.4785 },
        { num: 47, x: 55.4868, y: 93.4785 },
        { num: 46, x: 84.0802, y: 93.8741 },
        { num: 45, x: 114.6549, y: 94.4892 },
        { num: 44, x: 144.7977, y: 95.0126 },
        { num: 43, x: 169.5968, y: 94.4892 },
        { num: 42, x: 198.8225, y: 94.4891 },
        { num: 41, x: 229.3609, y: 94.489 },
        { num: 31, x: 256.9392, y: 94.4892 },
        { num: 33, x: 316.4399, y: 94.4892 },
        { num: 32, x: 286.082, y: 95.0127 },
        { num: 34, x: 343.2068, y: 95.0127 },
        { num: 35, x: 373.6457, y: 95.5 },
        { num: 36, x: 403.5746, y: 95.4998 },
        { num: 37, x: 433.4513, y: 95.4999 },
        { num: 38, x: 464.0921, y: 95.4999 },
    ]

    const DIVIDER_LINES = [
        { x: 440.2, y: 63.4 },
        { x: 412.5, y: 62.8 },
        { x: 382.7, y: 63.1 },
        { x: 355, y: 62.5 },
        { x: 323.2, y: 62.4 },
        { x: 295.5, y: 61.8 },
        { x: 265.7, y: 62.1 },
        { x: 238, y: 61.5 },
        { x: 207.8, y: 62.4 },
        { x: 180.1, y: 61.9 },
        { x: 150.3, y: 62.1 },
        { x: 122.6, y: 61.6 },
        { x: 91.7, y: 61.4 },
        { x: 33.3, y: 61.1 },
        { x: 63.1, y: 60.8 },
    ]

    const ADULT_STRIP = { x: 12.7, width: 455.6 }

    // The baby chart reuses the adult artwork with the back molars dropped, so it
    // is cropped to the teeth that are left - otherwise it would draw a small
    // cluster of teeth with a wide empty gap on both sides.
    const CHART_WIDTH = 481
    const CHART_HEIGHT = 150
    const ADULT_VIEW_BOX = `0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`
    const PRIMARY_X = 84
    const PRIMARY_WIDTH = 308
    const PRIMARY_VIEW_BOX = `${PRIMARY_X} 0 ${PRIMARY_WIDTH} ${CHART_HEIGHT}`
    const PRIMARY_STRIP = { start: 91.7, end: 383.6 }

    export default {
        setup() {
            // The clinic can label baby teeth as 51-85, A-T or A-E (see Settings)
            const { babyTeethNotation } = useClinicSettings()
            return { babyTeethNotation }
        },
        props: {
            tooth_num: {
                type: Array,
                default: () => []
            },
            id: Number,
            categories: {
                type: Array,
                default: () => []
            },
            patientCases: {
                type: Array,
                default: () => []
            },
            patientData: {
                type: Object,
                default: () => ({})
            },
            // 'permanent' -> adult chart (FDI 11-48), 'primary' -> baby chart (FDI 51-85)
            dentition: {
                type: String,
                default: 'permanent'
            },
            // Baby teeth labels: 'fdi' | 'universal' | 'palmer'. Left empty the
            // chart follows the clinic setting.
            notation: {
                type: String,
                default: ''
            }
        },
        data() {
            return {
                active: '',
                showContextMenu: false,
                contextTooth: null,
                selectedTeeth: [], // Track selected teeth for coloring
                
                // Simple category selection
                categorySearch: '',
                
                // Cache configuration
                cacheConfig: {
                    caseCategories: {
                        key: 'case_categories_cache',
                        ttl: 60 * 60 * 1000, // 1 hour
                    }
                },
                loadingCategories: false,
                localCategories: []
            }
        },
        mounted() {
            // Load categories from cache or parent
            this.loadCategories();
            
            // Add touch support for mobile devices
            this.$nextTick(() => {
                this.bindToothTouchHandlers();
            });
        },
        
        beforeDestroy() {
            // No event listeners to clean up with v-bottom-sheet
        },
        computed: {
            isPrimaryDentition() {
                return this.dentition === 'primary';
            },

            viewBox() {
                return this.isPrimaryDentition ? PRIMARY_VIEW_BOX : ADULT_VIEW_BOX;
            },

            // The baby chart is cropped to fewer teeth, so filling the full width
            // would draw each tooth much larger than on the adult chart. Narrow the
            // element by the same ratio instead - the teeth then keep the adult size
            // and the shorter arch sits centered.
            chartScale() {
                if (!this.isPrimaryDentition) return '100%';
                return `${(PRIMARY_WIDTH / CHART_WIDTH) * 100}%`;
            },

            // Labels of the teeth this chart draws, numbered for the dentition shown
            toothLabels() {
                return TOOTH_LABELS
                    .filter(label => this.showTooth(label.num))
                    .map(label => ({ ...label, display: this.formatToothLabel(this.toFdi(label.num)) }));
            },

            dividerLines() {
                if (!this.isPrimaryDentition) return DIVIDER_LINES;
                return DIVIDER_LINES.filter(d => d.x >= PRIMARY_STRIP.start && d.x <= PRIMARY_STRIP.end);
            },

            numberStrip() {
                if (!this.isPrimaryDentition) return ADULT_STRIP;
                return { x: PRIMARY_STRIP.start, width: PRIMARY_STRIP.end - PRIMARY_STRIP.start };
            },

            textClass() {
                return this.$vuetify && this.$vuetify.theme.dark ? 'text-dark-mode' : 'text-light-mode';
            },
            // Use cached categories or prop categories
            availableCategories() {
                return this.localCategories.length > 0 ? this.localCategories : this.categories;
            },
            
            // Categories to display in bottom sheet (filtered by search, tooth-only)
            displayedCategories() {
                const cats = (this.localCategories.length > 0 ? this.localCategories : this.categories)
                    .filter(c => !c.without_detect_tooth);
                if (!cats || !Array.isArray(cats)) return [];
                if (!this.categorySearch || !this.categorySearch.trim()) return cats;

                const search = this.categorySearch.toLowerCase().trim();
                return cats.filter(cat => {
                    const name = (cat.name || cat.name_ar || '').toLowerCase();
                    return name.includes(search);
                });
            },

            categoriesWithoutTooth() {
                const cats = this.localCategories.length > 0 ? this.localCategories : this.categories;
                if (!cats || !Array.isArray(cats)) return [];
                return cats.filter(c => c.without_detect_tooth === true);
            }
        },
        methods: {
            // Teeth dropped by the baby chart are re-created when the adult chart
            // comes back, so they are bound again - the flag keeps the teeth that
            // stayed in place from getting a second listener.
            bindToothTouchHandlers() {
                const toothElements = this.$el.querySelectorAll('path.comon');
                toothElements.forEach(element => {
                    const toothId = element.id;
                    if (!toothId || element.dataset.touchBound) return;
                    element.dataset.touchBound = 'true';

                    // Add touchstart event listener for mobile support
                    element.addEventListener('touchstart', (event) => {
                        event.preventDefault();
                        this.reply_click(parseInt(toothId), event);
                    }, { passive: false });

                    // Add touchend event listener to prevent double firing
                    element.addEventListener('touchend', (event) => {
                        event.preventDefault();
                    }, { passive: false });
                });
            },

            // A baby mouth has no back molars, so positions 6-8 are dropped there
            showTooth(toothNum) {
                return !this.isPrimaryDentition || (toothNum % 10) <= PRIMARY_LAST_POSITION;
            },

            // The artwork is numbered for adults - baby cases are stored with the
            // primary FDI number, which is the adult one shifted a quadrant (11 -> 51)
            toFdi(toothNum) {
                return this.isPrimaryDentition ? toothNum + PRIMARY_TOOTH_OFFSET : toothNum;
            },

            // Label drawn under the tooth - baby teeth may be lettered instead
            formatToothLabel(toothNum) {
                return formatToothNumber(toothNum, this.notation || this.babyTeethNotation);
            },

            // Cache management methods
            setCache(key, data, ttl = 60 * 60 * 1000) {
                const cacheItem = {
                    data: data,
                    timestamp: Date.now(),
                    ttl: ttl
                };
                try {
                    localStorage.setItem(key, JSON.stringify(cacheItem));
                } catch (error) {
                    console.warn('Failed to set cache:', error);
                }
            },

            getCache(key) {
                try {
                    const cached = localStorage.getItem(key);
                    if (!cached) return null;

                    const cacheItem = JSON.parse(cached);
                    const now = Date.now();
                    
                    // Check if cache is expired
                    if (now - cacheItem.timestamp > cacheItem.ttl) {
                        localStorage.removeItem(key);
                        return null;
                    }
                    
                    return cacheItem.data;
                } catch (error) {
                    console.warn('Failed to get cache:', error);
                    return null;
                }
            },

            clearCache(key) {
                try {
                    localStorage.removeItem(key);
                } catch (error) {
                    console.warn('Failed to clear cache:', error);
                }
            },

            // Load categories from cache or fetch from API
            loadCategories() {
                // First check if categories are passed as props
                if (this.categories && this.categories.length > 0) {
                    this.localCategories = [...this.categories];
                    this.filteredCategories = [...this.categories];
                    return;
                }

                // Check cache
                const cached = this.getCache(this.cacheConfig.caseCategories.key);
                if (cached) {
                    this.localCategories = cached;
                    this.filteredCategories = cached;
                    return;
                }

                // If no cache and no props, fetch from API
                this.getCaseCategories();
            },

            getCaseCategories() {
                this.loadingCategories = true;
                
                const cached = this.getCache(this.cacheConfig.caseCategories.key);
                if (cached) {
                    this.localCategories = cached;
                    this.loadingCategories = false;
                    return;
                }

                // Make API request
                const apiRequest = this.$axios || this.$http || window.axios;
                if (!apiRequest) {
                    console.warn('No HTTP client available for API request');
                    this.loadingCategories = false;
                    return;
                }

                apiRequest.get('case-categories', {
                    headers: {
                        "Content-Type": "application/json",
                        Accept: "application/json",
                        Authorization: "Bearer " + (this.$store?.state?.AdminInfo?.token || '')
                    }
                })
                .then(res => {
                    this.localCategories = res.data;
                    this.filteredCategories = res.data;
                    this.setCache(this.cacheConfig.caseCategories.key, res.data, this.cacheConfig.caseCategories.ttl);
                    this.loadingCategories = false;
                })
                .catch(() => {
                    this.loadingCategories = false;
                    // Try to use expired cache as fallback
                    const expiredCache = localStorage.getItem(this.cacheConfig.caseCategories.key);
                    if (expiredCache) {
                        try {
                            this.localCategories = JSON.parse(expiredCache).data;
                        } catch (e) {
                            console.warn('Failed to parse expired cache');
                        }
                    }
                });
            },

            isFormElement(element) {
                if (!element) return false;
                if (element.tagName === 'INPUT' || 
                    element.tagName === 'TEXTAREA' || 
                    element.tagName === 'SELECT' || 
                    element.tagName === 'BUTTON' ||
                    element.tagName === 'FORM') {
                    return true;
                }
                if (element.classList && (
                    element.classList.contains('v-input') ||
                    element.classList.contains('v-text-field') ||
                    element.classList.contains('v-select') ||
                    element.classList.contains('v-data-table') ||
                    element.classList.contains('price-input') ||
                    element.classList.contains('v-btn') ||
                    element.classList.contains('v-form')
                )) {
                    return true;
                }
                if (element.closest && (
                    element.closest('.v-input') ||
                    element.closest('.v-text-field') ||
                    element.closest('.v-select') ||
                    element.closest('.v-data-table') ||
                    element.closest('.price-input') ||
                    element.closest('.v-form') ||
                    element.closest('form')
                )) {
                    return true;
                }
                return false;
            },

            // Check if a tooth has existing cases from the API
            hasExistingCase(toothNum) {
                if (!this.patientCases || !this.patientCases.length) return false;
                return this.patientCases.some(c => {
                    // Check tooth_number field
                    if (c.tooth_number && parseInt(c.tooth_number) === parseInt(toothNum)) return true;
                    
                    // Check tooth_num field (can be JSON array or single value)
                    if (c.tooth_num) {
                        try {
                            const nums = JSON.parse(c.tooth_num);
                            if (Array.isArray(nums)) {
                                return nums.includes(parseInt(toothNum)) || nums.includes(String(toothNum));
                            }
                            return parseInt(nums) === parseInt(toothNum);
                        } catch {
                            return parseInt(c.tooth_num) === parseInt(toothNum) || c.tooth_num === String(toothNum);
                        }
                    }
                    return false;
                });
            },

            // Check if a tooth is active (selected or has existing cases)
            isToothActive(toothNum) {
                const num = this.toFdi(toothNum);
                return this.selectedTeeth.includes(num) || this.hasExistingCase(num);
            },

            preventGlobalContextMenu(event) {
                const activeElement = document.activeElement;
                if (activeElement && this.isFormElement(activeElement)) {
                    event.preventDefault();
                    event.stopPropagation();
                    event.stopImmediatePropagation();
                    return false;
                }
                if (!event.target || !event.target.classList.contains('comon')) {
                    event.preventDefault();
                    event.stopPropagation();
                    return false;
                }
            },

            reply_click(id, event) {
                // Simple click handler to show bottom sheet with categories
                event.preventDefault();
                event.stopPropagation();
                
                this.contextTooth = this.toFdi(id);
                this.categorySearch = ''; // Reset search
                this.showContextMenu = true;
            },

            right_click(id, event) {
                // Enhanced right-click handler for teeth with mobile support
                event.preventDefault();
                event.stopPropagation();
                
                this.contextTooth = this.toFdi(id);
                this.showContextMenu = true;
                
                // Get coordinates from event (works for both mouse and touch)
                let x = event.clientX || (event.touches && event.touches[0] ? event.touches[0].clientX : event.pageX);
                let y = event.clientY || (event.touches && event.touches[0] ? event.touches[0].clientY : event.pageY);
                
                const menuWidth = 180;
                const menuHeight = 300;
                const viewportWidth = window.innerWidth;
                const viewportHeight = window.innerHeight;
                
                // Adjust position to keep menu in viewport
                if (x + menuWidth > viewportWidth) x = viewportWidth - menuWidth - 10;
                if (y + menuHeight > viewportHeight) y = viewportHeight - menuHeight - 10;
                x = Math.max(10, x);
                y = Math.max(10, y);
                
                this.contextMenuStyle.top = y + 'px';
                this.contextMenuStyle.left = x + 'px';
                this.contextMenuStyle.display = 'block';
                
                // Add event listeners to hide menu when clicking elsewhere
                setTimeout(() => {
                    document.addEventListener('click', this.hideContextMenu);
                    document.addEventListener('touchstart', this.hideContextMenu);
                }, 100);
            },

            hideContextMenu() {
                this.showContextMenu = false;
                this.contextTooth = null;
                this.categorySearch = '';
            },

            selectCategoryWithoutTooth(category) {
                this.$emit('case-added', {
                    toothNumber: null,
                    toothId: null,
                    category: category,
                    categoryName: category.name || category.name_ar || '',
                    timestamp: Date.now()
                });

                // Show success feedback
                if (this.$swal) {
                    this.$swal.fire({
                        icon: 'success',
                        title: 'تمت الإضافة',
                        text: `تم إضافة ${category.name || category.name_ar}`,
                        timer: 2000,
                        showConfirmButton: false
                    });
                }
            },

            selectCategory(category) {
              if (this.contextTooth && category) {
                const toothNum = this.contextTooth;
                const toothId = `tooth-${toothNum}`;
                
                // Add the tooth to selectedTeeth array for visual highlighting
                if (!this.selectedTeeth.includes(toothNum)) {
                  this.selectedTeeth.push(toothNum);
                }
                
                // Emit event for parent to add case row with proper structure
                this.$emit('case-added', {
                  toothNumber: toothNum,
                  toothId: toothId,
                  category: category,
                  categoryName: category.name || category.name_ar || '',
                  timestamp: Date.now()
                });
                
                // Show success feedback
                if (this.$swal) {
                  this.$swal.fire({
                    icon: 'success',
                    title: 'تمت الإضافة',
                    text: `تم إضافة ${category.name || category.name_ar} للسن ${toothNum}`,
                    timer: 2000,
                    showConfirmButton: false,
                    toast: true,
                    position: 'top-end'
                  });
                }
                
                // Hide bottom sheet
                this.hideContextMenu();
              }
            },
            
            // Enhanced category filtering
            filterCategories(item, queryText) {
              if (!queryText) return true;
              const search = queryText.toLowerCase().trim();
              const name = (item.name || item.name_ar || '').toLowerCase();
              return name.includes(search);
            },
            
            // Format currency for display
            formatCurrency(amount) {
              if (!amount) return '0';
              return new Intl.NumberFormat('ar-EG').format(amount);
            },
            
            // Refresh categories from API
            async refreshCategories() {
              this.loadingCategories = true;
              try {
                // Clear cache first
                this.clearCache(this.cacheConfig.caseCategories.key);
                
                // Fetch fresh data
                await this.getCaseCategories();
                
                // Show success message
                if (this.$swal) {
                  this.$swal.fire({
                    icon: 'success',
                    title: 'تم التحديث',
                    text: 'تم تحديث قائمة فئات العمليات بنجاح',
                    timer: 2000,
                    showConfirmButton: false,
                    toast: true,
                    position: 'top-end'
                  });
                }
              } catch (error) {
                console.error('Error refreshing categories:', error);
                if (this.$swal) {
                  this.$swal.fire({
                    icon: 'error',
                    title: 'خطأ',
                    text: 'فشل في تحديث قائمة فئات العمليات',
                    confirmButtonText: 'موافق'
                  });
                }
              } finally {
                this.loadingCategories = false;
              }
            },
            
            // Add new category
            async addNewCategory() {
              if (!this.newCategory.name_ar.trim()) {
                return;
              }
              
              this.savingNewCategory = true;
              try {
                const apiRequest = this.$axios || this.$http || window.axios;
                if (!apiRequest) {
                  throw new Error('No HTTP client available');
                }
                
                const categoryData = {
                  name_ar: this.newCategory.name_ar.trim()
                };
                
                const response = await apiRequest.post('case-categories', categoryData, {
                  headers: {
                    "Content-Type": "application/json",
                    Accept: "application/json",
                    Authorization: "Bearer " + (this.$store?.state?.AdminInfo?.token || '')
                  }
                });
                
                // Add new category to local array
                const newCat = response.data.data || response.data;
                this.localCategories.push(newCat);
                this.filteredCategories = [...this.localCategories];
                
                // Update cache
                this.setCache(this.cacheConfig.caseCategories.key, this.localCategories, this.cacheConfig.caseCategories.ttl);
                
                // Emit event to parent to refresh their categories
                this.$emit('category-added', newCat);
                
                // Show success message and reset form
                if (this.$swal) {
                  this.$swal.fire({
                    icon: 'success',
                    title: 'نجح',
                    text: 'تم إضافة فئة العملية بنجاح',
                    timer: 2000,
                    showConfirmButton: false,
                    toast: true,
                    position: 'top-end'
                  });
                }
                
                this.cancelAddCategory();
                
              } catch (error) {
                console.error('Error adding new category:', error);
                
                let errorMessage = 'فشل في إضافة فئة العملية';
                if (error.response && error.response.data && error.response.data.message) {
                  errorMessage = error.response.data.message;
                }
                
                if (this.$swal) {
                  this.$swal.fire({
                    icon: 'error',
                    title: 'خطأ',
                    text: errorMessage,
                    confirmButtonText: 'موافق'
                  });
                }
              } finally {
                this.savingNewCategory = false;
              }
            },
            
            // Cancel add category form
            cancelAddCategory() {
              this.showAddCategoryForm = false;
              this.newCategory = {
                name_ar: ''
              };
              this.validNewCategory = false;
              if (this.$refs.newCategoryForm) {
                this.$refs.newCategoryForm.reset();
              }
            },
        },
        
        watch: {
            dentition() {
                this.selectedTeeth = [];
                this.$nextTick(() => {
                    this.bindToothTouchHandlers();
                });
            },

            // Watch for changes in categories prop
            categories: {
                handler(newCategories) {
                    if (newCategories && newCategories.length > 0) {
                        this.localCategories = [...newCategories];
                        this.filteredCategories = [...newCategories];
                        // Cache the new categories
                        this.setCache(this.cacheConfig.caseCategories.key, newCategories, this.cacheConfig.caseCategories.ttl);
                    }
                },
                deep: true,
                immediate: true
            },
            
            // Update filtered categories when search changes
            categorySearch: {
                handler() {
                    this.filteredCategories = this.searchFilteredCategories;
                },
                immediate: true
            }
        }
    }
</script>